...
did:function did(i){return document.getElementById(i);}
	...
	,bldIndiLevels:function bldIndiLevels(p,o,sectrName){
		var t=bldIndiLevels;
		...
		function cb(o,sectrName,p,lbl){
			var lvl='indi'
			,code=o&&o.meta?o.meta['Series code']:'code'+o
			//,n=x[sectrName]
		...
		{var x=o[i]
			...
			,btn=sys.dce('span',li,{t:i//+': '+formatSeries(o,'Sector'),
				,clk:t.f	})
		...
		/*
		 bldIndiLevels:function bldIndiLevels(p,o,txt,sectrName){
		 var t=bldIndiLevels;
		 if(!t.f)t.f=function indiLevelsClk(evt){
		 var s=sys.nxt(this,'UL','OL').style,d=s.display;
		 s.display=d=='none'?'block':'none';
		 }//function indiLevelsClk
		 function formatTooltip(o){
		 var b=[];for(var i in o)
		 if(i!='path' && i!='rowIx')
		 ...
		 ,clss:'filter'
		 ,chng:sys.bldLists.chngFltr})
		 inp.setAttribute('entty',inp.entty={e:'indi',n:code})
		 return inp;
		 }
		 function isExpandable(o){
		 var r= o&&o.meta&&(d3.keys(o).length>1);
		 return r;}
		 var li=sys.dce('li',p,//TODO: generate checkbox and label
		 {attribs:{meta:o.meta,title:formatTooltip(o.meta)}
		 ,styl:isExpandable(o)
		 ?'list-style-image:expandable.png'
		 :'list-style-type:circle'})
		 ,btn=sys.dce('span',li,{t:txt//+': '+formatSeries(o,'Sector'),
		 ,clk:t.f	})
		 ,inp=cb(o,'gci',li)
		 ,ul=sys.dce('ul',li,{styl:'display:none'});
		 for(var i in o)if(i!='meta')
	...
	*/
	...
	,bldFltrPanel:function bldFilterPanel(d,title,id){
		//add search filter
		var inp=sys.dci(id+'-searchFilter',d,{typ:'search',lbl:'filter'})
		inp.oninput=
		...
		{//console.log('filter.oninput:',evt,this)
			...
			function onInput_new(evt)
			{//console.log('filter.oninput:',evt,this)
				/*function txt(e){
				 var x=e,r;
				 if(x.nodeType==Element.TEXT_NODE && x.data.trim().length>0)
				 return x.data;
				 x=e.firstChild;
				 while(x)
				 {if(x.nodeType==Element.TEXT_NODE && x.data.trim().length>0)
				 ...
				 function srch(txt,x,a){
				 if(!x)return a;
				 if(x.nodeType==Element.TEXT_NODE && x.data.toLowerCase().indexOf(txt)!=-1)
				 a.push(x);//return x;
				 if(x.title && x.title.toLowerCase().indexOf(txt)!=-1)
				 a.push(x);//return x;
				 x=x.firstChild;
				 while(x)
				 {	srch(txt,x,a);
				...
				function findNdestroy(n,txt,a){
					function clon(e){
						var c=e.cloneNode(true),ba,bb
						,a=c.getElementsByTagName('ol')
						,b=c.getElementsByTagName('ul');
						while((ba=a && a.length>0)||(bb=b && b.length>0))
						{if(ba){
					...
					var nxt=sys.nxt
					if(!n)return a;
					if(n.nodeType==Element.TEXT_NODE && n.data.toLowerCase().indexOf(txt)!=-1)
					{while(n && n.nodeName!='LI')
						...
						if(n.title && n.title.toLowerCase().indexOf(txt)!=-1)
						{
							a.push(n);
							return a;
						}else
							n=n.firstChild;
					while(n)
					{	findNdestroy(txt,n,a);
						n=n.nextSibling;
					}
					return a;
				}//function findNdestroy
				var str=this.value,l=str.toLowerCase()
				,y,v,i,j,k
				,results=this.parentElement.results
				,x,a
				results.innerHTML=''
				if(str && str.length>1)
				{a=srch(l,this.parentElement,[])
				 for(j=0;j<a.length;j++)
				{x=a[j];
					v=txt(x)//x.firstChild.nextElementSibling.firstChild.data
					i=v.toLowerCase().indexOf(l)
					
					if(i!=-1)
					{y=clon(x)//x.cloneNode(true);
						y.firstChild.nextElementSibling.innerHTML=v.substring(0,i)
						+"<i style=\"text-decoration: underline;font-weight: 800; background-color: yellow; \">"
						+str+"</i>"+v.substring(i+str.length)
						y.firstChild.onchange=function(){
	...
	,bldLists:function(sectrName,sectrLookup){//sectrLookup is <sector>.lookup
		var div=sys.did(sectrName+'.lookups');sectrName=sectrName.toLowerCase();
		...
		var panelsTitles={cntry:'Countries'//,indi:'All Indicators',code:'All Indicators'
			,yr:'Years',vars:'Variables',format:'Formats',chrtTyp:'Chart Type',indiLvls:'Indicators'}//,dbLvls:['indi','cntry','yr','vars']//'sectr',
		{var sectr=xls[sectrName]//||xls[sectrName.toLowerCase()]
		 if(sectr){var il=db.sectors[sectrName].indiLevels;
		 if(il){
			...
			d.ol=sys.bldFltrPanel(d,panelsTitles.indiLvls,sectrName+'-indiLvls')
			 sys.bldIndiLevels(d.ol,il,sectrName)
		}}}
		for(var lvl in sectrLookup)if(lvl!='code')
		{	var x=sectrLookup[lvl],d=sys.dce(
				 'div',div,{clss:'stylDiv',id:'div-'+lvl})
				d.ol=sys.bldFltrPanel(d,panelsTitles[lvl],sectrName+'-'+lvl)
			if(lvl=='code')
			{	/*var id=sectrName+'-'+lvl+'-';d.ol=sys.bldFltrPanel(d,panelsTitles[lvl],id)
				for(var e in x)
				{	var n=x[e],m=sectrName+'.'+e+'.'+n.code// changed 2016-06-29-08-50 i to e
		...
		{var chartTypes=['Line Chart','Line Chart 2','Bar Chart','Horizontal Bar Chart','Spider Chart','Radar Chart']
		...
		d.ol=sys.bldFltrPanel(d,panelsTitles.format,sectrName+'-format')
		for(var i in a)
			for(var j in a)
				if(j!=i)
					for(var k in a)
						if(k!=i && k!=j)
							for(var l in a)
								if(l!=i && l!=j && l!=k)
								{c=[i,j,k,l];m=c.join(',');
									inp=sys.dci('lvls',sys.dce('li',d.ol,{clss:'stylLI'})
